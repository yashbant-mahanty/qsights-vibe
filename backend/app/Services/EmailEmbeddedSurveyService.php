...
class EmailEmbeddedSurveyService
{
    ...
    public function sendEmbeddedSurvey(
        ...
        // Log received email config
        Log::info('EmailEmbeddedSurveyService - Received email config', [
        ...
        Log::info('EmailEmbeddedSurveyService - Processed config values', [
            'subject' => $subject,
            'fromName' => $fromName,
            'preheader' => $preheader,
            'headerText' => $headerText,
            'headerText_length' => $headerText ? strlen($headerText) : 0,
            'footerText' => $footerText,
            'footerText_length' => $footerText ? strlen($footerText) : 0,
            'headerText_empty_check' => empty($headerText) ? 'YES' : 'NO',
            'footerText_empty_check' => empty($footerText) ? 'YES' : 'NO',
        ]);
     ...
     */
    private function getParticipantName(string $email, string $activityId): string
    {
        // Find participant through the activity's participants relationship
        $activity = \App\Models\Activity::find($activityId);
        if ($activity) {
            $participant = $activity->participants()->where('email', $email)->first();
            if ($participant) {
                return $participant->name;
            }
        }
        
        // Fallback: try to find participant by email alone
