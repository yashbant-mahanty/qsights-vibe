    ...
    private function scheduleNewJoineeEvaluation($staffId, $validated, $programId, $user)
    {
        try {
            ...
            try {
                ...
                $subject = "Trainee Evaluation Required - {$staff->name}";
                $body = "
                    <p>Hello {$evaluator->name},</p>
                    <p>Your new team member <strong>{$staff->name}</strong> has completed {$triggerDays} days with the company. Please evaluate their performance using the <strong>{$questionnaire->name}</strong> form.</p>
                    <p><strong>Details:</strong></p>
                    <ul>
                        <li>Employee: {$staff->name} ({$staff->email})</li>
                        <li>Joining Date: {$staff->joining_date}</li>
                        <li>Evaluation Period: {$triggerDays} days</li>
                        <li>Due Date: {$endDate->format('Y-m-d')}</li>
                    </ul>
                    <p>Please complete the evaluation by {$endDate->format('Y-m-d')}.</p>
                ...
                \Log::info('Email sent for new joinee evaluation', [
                    'to' => $evaluator->email,
                    'staff_name' => $staff->name
                ]);
            } catch (\Exception $emailError) {
                \Log::error('Failed to send email for new joinee evaluation', [
