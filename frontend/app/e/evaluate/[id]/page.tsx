...
interface Question {
  id?: number;
  title?: string;  // API sends "title"
  question?: string;  // Legacy field
  type: 'rating' | 'mcq' | 'text' | 'radio' | 'checkbox' | 'textarea' | 'scale';
  options?: string[];
  scale?: number;
  description?: string;
  is_required?: boolean;
...
interface Subordinate {
  ...
  staff_name?: string;  // API sends this
  email?: string;  // Legacy field
  staff_email?: string;  // API sends this
  employee_id?: string;
}
    ...
    const fetchEvaluation = async () => {
      ...
      try {
        ...
        if (data.success && data.evaluation) {
          ...
          // Normalize subordinates to ensure consistent field names
          const normalizedEvaluation = {
            ...data.evaluation,
            subordinates: data.evaluation.subordinates.map((sub: any) => ({
              id: sub.id || sub.staff_id,
              staff_id: sub.staff_id || sub.id,
              name: sub.name || sub.staff_name,
              staff_name: sub.staff_name || sub.name,
          ...
          normalizedEvaluation.subordinates.forEach((sub: Subordinate) => {
            const subId = sub.id || sub.staff_id || '';
            initialResponses[subId] = {};
          });
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex gap-2 overflow-x-auto py-3">
            {evaluation?.subordinates.map((sub, index) => {
              const subId = sub.id || sub.staff_id || '';
              const subResponses = responses[subId] || {};
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            ...
            <div className="space-y-8">
              {questions.map((question, qIndex) => {
                const questionText = question.title || question.question || 'No question text';
                ...
                <div key={qIndex} className="border-b pb-6 last:border-b-0">
                  ...
                  {/* Radio/MCQ type */}
                  {(questionType === 'mcq' || questionType === 'radio' || questionType === 'checkbox') && question.options && (
