...
interface EvaluationAdminLayoutProps {
      ...
      >
        ...
        <nav className="p-4 space-y-1 overflow-y-auto h-[calc(100%-4rem)]">
          {sidebarItems.map((item, index) => {
            ...
            // For other routes: check if pathname starts with the route (to handle sub-routes like /questionnaires/19)
            const isActive = item.href === '/evaluation-admin' 
              ? pathname === item.href 
                    ...
                    <div className="bg-gradient-to-br from-qsights-blue to-blue-600 px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-blue-100 text-xs lowercase truncate">{currentUser?.role?.replace('_', '-') || 'evaluation-admin'}</p>
                        </div>
                      </div>
