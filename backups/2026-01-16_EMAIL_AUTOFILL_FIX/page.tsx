...
export default function TakeActivityPage() {
  ...
  // URL parameters - handle both access tokens (from emails) and legacy parameters
  const urlToken = searchParams.get("token"); // Can be participant access token OR encrypted link token
  ...
  // Token is available from URL - we'll determine its type during validation
  const token = urlToken; // Always use the URL token for access token validation
  
  // Determine link type from legacy params
  const linkType = legacyType;
  const isPreview = legacyPreview;
